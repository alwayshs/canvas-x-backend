// ===================================================================
// server.js (메인 서버 프로그램) - 최종 버전 (CORS origin 슬래시 제거, whitelist 체크 수정)
// ===================================================================
...
const corsOptions = {
  origin: function (origin, callback) {
    // origin이 없을 경우 (Postman, 서버-to-서버 요청 등) 허용
    if (!origin) return callback(null, true);

    // whitelist에 포함되어 있는지 확인
...
app.get('/api/auctions/:id', async (req, res) => {
  try {
    ...
    const bidsResult = await db.query(
      "SELECT b.amount, u.nickname FROM bids b JOIN users u ON b.user_id = u.id WHERE b.auction_id = $1 ORDER BY b.created_at DESC",
      [req.params.id]
    );
